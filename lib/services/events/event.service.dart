...
class EventService {
  ...
  Future<List<Event>> fetchEventsByUserId(String id) async {
    try {
      final response = await http
          .get(Uri.parse('${ApiConfig.apiBaseUrl}/events/user/all/$id'))
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Falha ao carregar Events do usuário: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<List<Event>> fetchEventsByAgencyID(String id) async {
    try {
      final response = await http
          .get(Uri.parse('${ApiConfig.apiBaseUrl}/events/agency/all/$id'))
          .timeout(Duration(seconds: 30));
  ...
  Future<void> createEvent({
    ...
    try {
      ...
      print(
          'Arquivos: ${request.files.map((file) => file.field + ": " + file.filename!).toList()}');
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Falha ao criar evento: ${response.statusCode}, $responseBody');
      }
    } catch (e) {
  ...
  Future<void> updateEvent(String id, Map<String, dynamic> updateData) async {
    try {
      final response = await http.put(
        Uri.parse('${ApiConfig.apiBaseUrl}/events/$id'),
      ...
      throw Exception('Erro ao atualizar o evento: $e');
    }
  }
}
